...
# Captura IDs de paciente no padrão OAS1_XXXX.
SUBJECT_ID_PATTERN = re.compile(r'(OAS1_\d{4})', flags=re.IGNORECASE)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--output-csv',
        type=Path,
        default=Path('reports/split_assignments_grouped.csv'),
...
def extract_subject_id(file_path: Path) -> str:
    # Extrai o ID do paciente para impedir vazamento entre conjuntos.
    match = SUBJECT_ID_PATTERN.search(file_path.name)
    if not match:
        raise ValueError(f'Não foi possível extrair subject_id de: {file_path.name}')
