...
def parse_args() -> argparse.Namespace:
    # Define parâmetros do experimento com Vision Transformer.
    parser = argparse.ArgumentParser(
        description='Treino de ViT (vit_b_16) com transfer learning usando CSV de split.',
    )
    parser.add_argument(
        '--split-csv',
        type=Path,
        default=Path('reports/split_assignments_grouped_binary.csv'),
        help='CSV com colunas filepath, label e split.',
    )
    parser.add_argument(
        '--output-dir',
        type=Path,
        default=Path('reports/vit_transfer_learning'),
...
def main() -> None:
    ...
    if best_state_dict is None:
        raise RuntimeError('Não foi possível salvar o melhor estado do modelo ViT.')
